...
void ImGuiImpl::render(RenderContext *ctx) {
	...
	ctx->setViewport(0, 0, ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y, 0.0f, 1.0f);
...
static void createDeviceObjects(Platform *platform, RenderContext *ctx) {
	...
	im_raster_state = ctx->createRasterState(true, true, false);

	// Create depth-stencil State
	im_depth_stencil_state = ctx->createDepthStencilState(false);
...
void ImGuiImpl::init(Platform *platform, PlatformWindow *window, Shader *shader) {
	ImGuiIO& io = ImGui::GetIO();
...
void ImGuiImpl::newFrame(Platform *platform, PlatformWindow *window, RenderContext *ctx, float delta) {
