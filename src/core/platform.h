...
struct Platform {
	...
	virtual bool copyFile(char *a, char *b);
	virtual FileTime getLastWriteTime(char *file);
	virtual u32 compareFileTime(FileTime *a, FileTime *b);
	
	virtual DirectoryContentsResult getDirectoryContents(const char *folder_path);
	...
	virtual void *reAlloc(void *data, u64 size);
	...
	virtual void processEvents(PlatformWindow *window, bool &requested_to_quit, bool &resized);
