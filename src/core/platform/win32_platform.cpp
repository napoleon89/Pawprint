...
DirectoryContentsResult Platform::getDirectoryContents(const char *folder_path) {
	WIN32_FIND_DATAA find_data;
	HANDLE file_handle = FindFirstFile(folder_path, &find_data);
	int count = 0;
	do {
		count++;
	...
	file_handle = FindFirstFile(folder_path, &find_data);

	char **result_chars = (char **)alloc(sizeof(char *) * count);
	int index = 0;
	do {
		size_t string_len = strlen(find_data.cFileName);
		char *ptr = (char *)alloc(sizeof(char) * (string_len+1));
		memcpy(ptr, find_data.cFileName, string_len);
		ptr[string_len] = '\0';
		result_chars[index++] = ptr;
	} while(FindNextFile(file_handle, &find_data));

	FindClose(file_handle);

	DirectoryContentsResult result = {};
	result.contents = result_chars;
	result.count = count;
	return result;
}
...
void Platform::processEvents(PlatformWindow *window, bool &requested_to_quit, bool &resized) {
	SDL_Event e;
	g_mouse_wheel = 0.0f;
